...
from customization_support_rle import get_KITTIMOTS_dicts, cover_areas_to_ignore
...
saving_enabled = True
saved_KITTIMOTS_dicts = './KITTIMOTS_dicts_rle.pkl'
...
for d in random.sample(dataset_dicts, 1):
    ...
    image.save('detectron2_pretrained_rle.png',)
...
os.makedirs(cfg.OUTPUT_DIR + '_rle', exist_ok=True)
...
cfg.MODEL.WEIGHTS = os.path.join(cfg.OUTPUT_DIR + '_rle', "model_final.pth")  # path to the model we just trained
cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.7   # set a custom testing threshold
predictor = DefaultPredictor(cfg)



saved_KITTIMOTS_dicts_val = './KITTIMOTS_dicts_rle_val.pkl'

if os.path.exists(saved_KITTIMOTS_dicts_val):
    with open(saved_KITTIMOTS_dicts_val, 'rb') as reader:
        print('Loading dataset dicts...')
        dataset_dicts_val = pickle.load(reader)
else:
    print('Generating dict for training...')
    dataset_dicts_val = get_KITTIMOTS_dicts('valid')
    if saving_enabled == True:
        with open(saved_KITTIMOTS_dicts_val, 'wb') as handle:
            print('Saving dataset dicts...')
            pickle.dump(dataset_dicts_val, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
for d in random.sample(dataset_dicts_val, 1):
    ...
    image = Image.fromarray(out.get_image()[:, :, ::-1])
    image.save('detectron2_trained_rle.png',)
